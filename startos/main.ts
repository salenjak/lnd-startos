  ...
  exec: {
    command: [
      ...
      [
        ...
        '',
        '# 11111111Wait for config file to exist',
...
'      sftp_host=$(grep -A 10 "\\[sftp\\]" /tmp/rclone.conf | grep "^host =" | cut -d= -f2 | tr -d " ")',
'      if [ -n "$sftp_host" ] && echo "$sftp_host" | grep -q "\\.onion$"; then',
'        echo "[$(date -Iseconds)] [SFTP] Using Tor (configured in rclone.conf)..." >&2',
'        if RCLONE_CONFIG=/tmp/rclone.conf timeout 45s rclone copy "$backup_file" "$remote" \\',
'          --log-level=INFO \\',
'          --contimeout=20s \\',
'          --timeout=40s \\',
'          --retries=1; then',
'          echo "[$(date -Iseconds)] [SFTP] ✅ Success" >&2',
'        else',
'          echo "[$(date -Iseconds)] [SFTP] ❌ Failed" >&2',
'        fi',
'        continue',
'      else',
'        # Clearnet SFTP: fast timeout',
'        if RCLONE_CONFIG=/tmp/rclone.conf timeout 12s rclone copy "$backup_file" "$remote" \\',
'          --log-level=INFO \\',
'          --contimeout=5s \\',
'          --timeout=10s \\',
'          --retries=1; then',
...
'    fi',
'',
'    # 👉 Handle WebDAV (Nextcloud) remotes',
...
'      echo "[$(date -Iseconds)] [RCLONE] Detected .onion address, using Tor proxy..." >&2',
'      if HTTP_PROXY=socks5://10.0.3.1:9050 HTTPS_PROXY=socks5://10.0.3.1:9050 ALL_PROXY=socks5://10.0.3.1:9050 RCLONE_CONFIG=/tmp/rclone.conf timeout 12s rclone copy "$backup_file" "$remote" \\',
'        --log-level=INFO \\',
'        --contimeout=5s \\',
'        --timeout=10s \\',
'        --retries=1 \\',
'        --no-check-certificate; then',
'        echo "[$(date -Iseconds)] [RCLONE] ✅ Success" >&2',
'      else',
'        echo "[$(date -Iseconds)] [RCLONE] ❌ Failed" >&2',
'      fi',
'    else',
'      # Normal clearnet (Dropbox, Google, SFTP IP, etc.)',
'      if RCLONE_CONFIG=/tmp/rclone.conf timeout 12s rclone copy "$backup_file" "$remote" \\',
'        --log-level=INFO \\',
'        --contimeout=5s \\',
'        --timeout=10s \\',
